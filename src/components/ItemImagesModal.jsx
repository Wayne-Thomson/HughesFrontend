...
const ItemImagesModal = ({ isOpen, onClose, item }) => {
  ...
  const fetchImage = async () => {
    setIsLoading(true)
    try {
      const response = await apiClient.get(`/api/item/image/${item._id}`)
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      formData.append('image', compressedBlob, 'item-image.jpg')

      // Send to backend
      toast.loading('Uploading image...', { id: 'upload' })
      const response = await apiClient.post(
        `/api/item/image/${item._id}`,
        formData,
        {
          headers: {
  ...
  const confirmDeleteImage = async () => {
    ...
    try {
      ...
      const response = await apiClient.delete(`/api/item/image/${item._id}`)

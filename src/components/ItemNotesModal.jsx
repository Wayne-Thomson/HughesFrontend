...
const ItemNotesModal = ({ isOpen, onClose, item, onItemUpdate }) => {
  ...
  useEffect(() => {
    if (isOpen && item) {
      const itemNotes = item.customNotes || ''
      setCustomNotes(itemNotes)
      setOriginalNotes(itemNotes)
    }
  }, [isOpen, item])
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Update the item in the parent's items array
        if (onItemUpdate) {
          onItemUpdate({ ...item, customNotes })
        }
      }
    } catch (error) {
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4" onClick={!isLoading ? onClose : undefined}>
        <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Item Notes</h2>
